...
final class RTMWhiteTileOverlay: MKTileOverlay {
    init() { super.init(urlTemplate: nil) }
}

/// Fills each tile flat white synchronously. Drawing here (rather than loading an
/// async tile image) is what keeps the white base from flickering over the streets.
...
extension RTMRoadType {
    ...
    var renderStyle: RenderStyle {
        let roadMeters: CGFloat = 11
        let trailMeters: CGFloat = 7
        switch self {
        case .primary, .residential, .service:
            return RenderStyle(color: .systemBlue, groundWidthMeters: roadMeters, dashPattern: nil)
        case .footway:
            return RenderStyle(color: .systemGreen, groundWidthMeters: roadMeters, dashPattern: nil)
        case .path:
            return RenderStyle(color: .systemGreen, groundWidthMeters: trailMeters, dashPattern: [10, 8])
        case .cycleway:
            return RenderStyle(color: .systemTeal, groundWidthMeters: trailMeters, dashPattern: [10, 8])
        case .steps:
